...
| --- | --- | --- | --- |
| `v0.8.0` | `1.0` | Python standard library only | MIT |
...
| Test evidence mesh | Reviews layered test partitions, child-suite ownership, stale/hidden/timeout evidence, background completion, and routine-vs-release gates |
...
### Test Evidence Mesh In v0.8.0

`v0.8.0` adds optional TestMesh helper APIs for splitting slow validation into
parent/child evidence layers. Use `TestMeshPlan`, `TestPartitionItem`,
`TestSuiteEvidence`, and `review_test_mesh(...)` when a parent test gate depends
on several child suites, background regressions, release-only checks, or
evidence that can become stale.
...
See [docs/test_evidence_mesh.md](docs/test_evidence_mesh.md) for the workflow
and API sketch.
...
- `test-mesh-template`: models parent/child test evidence for slow routine
  regressions, background suites, and release-only gates.
...
| Test evidence mesh | 检查测试分区、子套件 ownership、stale/hidden/timeout 证据、后台完成证据，以及 routine/release gate |
...
### v0.8.0 的 Test Evidence Mesh

`v0.8.0` 增加了 TestMesh helper API，用来把很慢、很大的测试验证拆成父子层级。
当一个父级测试 gate 依赖多个子套件、后台回归、release-only 检查，或者依赖可能
变 stale 的测试证据时，可以用 `TestMeshPlan`、`TestPartitionItem`、
`TestSuiteEvidence` 和 `review_test_mesh(...)`。
...
- `test-mesh-template`：用于把慢测试、后台测试和 release-only gate 拆成父子测试证据层。
