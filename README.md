...
| `flowguard-development-process-flow` | staged work, multi-skill setup, evidence freshness, or several outcome-equivalent routes need a lower-rework process choice; ordinary work skips optimizer ceremony |
...
All seventeen bundled skills use `skillguard.contract_source.v2`, bind one
existing FlowGuard owner/model projection, and carry a `contract_mapped` depth
profile. A `contract-depth-pass` proves that the prompt, native route, checks,
obligations, and positive/shallow calibration declarations line up; it is not
an execution-depth receipt and does not create a SkillGuard-owned domain route.
...
| `flowguard-development-process-flow` | staged work、multi-skill setup、证据新鲜度，或多条结果等价路线需要选择返工更少的流程；普通工作不增加优化仪式 |
...
内置的 17 个技能现在全部使用 `skillguard.contract_source.v2`，各自只绑定一个
既有 FlowGuard owner 和模型投影，并带有 `contract_mapped` 深度说明。
`contract-depth-pass` 只证明提示词、原生路线、检查、义务和正反例校准声明彼此对齐；
它不是实际执行深度回执，也不会新增一条由 SkillGuard 控制的业务路线。
