...
| --- | --- | --- | --- |
| `v0.16.0` | `1.0` | Python standard library only | MIT |
...
- **UI Text Hierarchy Blueprint:** review visible and assistive UI text by
  state, region, role, semantic key, owner, priority, duplication rationale,
  and warning/error escalation before copy or implementation flattens the
  screen into competing prose.
...
| UI text hierarchy blueprint | Inventories headings, labels, helper text, status text, empty/loading/success/failure text, CTA text, warnings, errors, and assistive text by UI state, region, owner, role, semantic key, priority, and duplication rationale |
...
### UI Text Hierarchy Blueprint In v0.16.0

`v0.16.0` adds UI Text Hierarchy Blueprint as a text-facing companion to UI
Flow Structure. UI Flow Structure answers what the interface can do and where
controls, regions, overlays, and stable placements belong. UI Text Hierarchy
Blueprint answers what the interface says in each state, which message is
primary, which labels or helper notes should stay local, which warnings or
errors must escalate, and which repeated text needs an explicit rationale.

Use it before copywriting or frontend implementation when a UI has dense
status text, competing headings, empty/loading/success/failure states,
destructive warnings, recovery hints, repeated semantic text, assistive text,
or multiple regions that could claim the same message. The blueprint should
inventory headings, labels, CTA text, helper text, status text, warnings,
errors, and state messages by state, region, owner, role, semantic key,
priority, and duplication rationale.

This route does not replace final copy, brand voice, localization, browser QA,
accessibility testing, or visual design. It produces a hierarchy contract those
workflows can consume after the text roles and state ownership are clear.

Use `UITypographyToken`, `UITextElement`, `UITextHierarchyBlueprint`, and
`review_ui_text_hierarchy(...)` when text roles and semantic typography tokens
should be derived from the reviewed UI interaction and structure model.
...
- **UI Text Hierarchy Blueprint：** 按 state、region、role、semantic key、owner、
  priority、重复理由和 warning/error 提升级别审查可见文字与 assistive text，
  避免 copy 或实现阶段把界面压平成互相竞争的 prose。
...
| UI text hierarchy blueprint | 按 UI state、region、owner、role、semantic key、priority 和重复理由梳理 heading、label、helper text、status text、empty/loading/success/failure text、CTA、warning、error 和 assistive text |
...
### v0.16.0 的 UI Text Hierarchy Blueprint

`v0.16.0` 增加 UI Text Hierarchy Blueprint，作为 UI Flow Structure 的文字层级同伴。
UI Flow Structure 回答界面能做什么，以及 control、region、overlay 和稳定位置应该放在哪里。
UI Text Hierarchy Blueprint 回答每个状态下界面应该说什么、哪条消息是 primary、哪些 label 或
helper note 只能保持 local、哪些 warning 或 error 必须提升层级，以及重复语义文字为什么可以存在。

当一个 UI 有密集状态文字、互相竞争的标题、empty/loading/success/failure 状态、破坏性警告、
恢复提示、重复语义文字、assistive text，或者多个 region 都可能承载同一条消息时，先使用这个
blueprint。它会按 state、region、owner、role、semantic key、priority 和 duplication rationale
梳理 heading、label、CTA、helper text、status text、warning、error 和状态消息。

这条路线不替代最终文案、品牌语气、本地化、browser QA、accessibility testing 或视觉设计。
它产出的是文字层级和状态 ownership 合同，供后续 copy、design、frontend 和 accessibility 工作消费。

当文字角色和语义 typography token 应该从已经审查过的 UI interaction model 和 structure
derivation 推导出来时，可以用 `UITypographyToken`、`UITextElement`、
`UITextHierarchyBlueprint` 和 `review_ui_text_hierarchy(...)`。
