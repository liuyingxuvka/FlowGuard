...
| --- | --- | --- | --- |
| `v0.10.0` | `1.0` | Python standard library only | MIT |
...
| Structure refactor mesh | Reviews large script/module splits, child-module ownership, public entrypoint compatibility, facades, dependency cycles, config drift, parity evidence, and routine-vs-release refactor gates |
| Skill Kernel route map | Keeps the Codex Skill as a compact router with sub-protocols for core modeling, ModelMesh, TestMesh, StructureMesh, model-miss, conformance/adoption, long checks, and framework upgrades |
...
This keeps FlowGuard usable as one Skill while making each sub-protocol easier
to maintain independently. Helper APIs such as `RiskIntent`, property
factories, templates, and mesh review functions remain package helpers, not
separate agent sub-skills.
...
entrypoints, missing facades, unsafe dependency cycles, config/default drift,
missing or stale behavior parity, insufficient evidence tiers, and the
difference between routine and release refactor confidence. It does not refactor
code; project adapters or agents collect source inventory, dependency, facade,
and parity evidence and pass it into FlowGuard.
...
- a compact Skill Kernel route map with sub-protocol references for core
  modeling, ModelMesh, TestMesh, StructureMesh, model-miss,
  conformance/adoption, long checks, and framework upgrades;
- TestMesh starter template for layered test validation evidence;
- StructureMesh starter template for large script/module split evidence;
...
| Structure refactor mesh | 检查大脚本/模块拆分、子模块 ownership、公开入口兼容、facade、依赖环、配置漂移、行为 parity，以及 routine/release refactor gate |
| Skill Kernel route map | 让 Codex Skill 保持成一个轻量 router，把 core modeling、ModelMesh、TestMesh、StructureMesh、model-miss、conformance/adoption、long checks 和 framework upgrades 分给子协议 |
...
这样 FlowGuard 仍然是一个统一 Skill，但每个子协议可以独立维护。`RiskIntent`、
property factories、templates 和 mesh review functions 仍然是 package helper，
不是单独触发的 agent 子技能。
...
effect/config 是否被重复拥有、公开入口是否被删除、facade 是否保留、是否引入危险依赖环、
config/default 是否漂移、behavior parity 是否缺失或 stale、证据等级是否不足，以及
routine confidence 和 release confidence 是否被区分。它不会替你移动代码；项目
adapter 或 agent 负责收集 source inventory、dependency、facade 和 parity evidence，
再把结构化证据交给 FlowGuard。
...
- 一个更轻量的 Skill Kernel 路由表，把 core modeling、ModelMesh、TestMesh、
  StructureMesh、model-miss、conformance/adoption、long checks 和 framework
  upgrades 分给独立 reference protocol；
- TestMesh 起步模板，用来管理父子测试层级和 routine/release 测试证据；
- StructureMesh 起步模板，用来管理大脚本/模块拆分、公开入口兼容和 routine/release refactor 证据；
