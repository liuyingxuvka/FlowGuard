...
<p align="center">
  <strong>An architecture simulator for checking software workflows before AI agents write code.</strong>
</p>
<!-- README HERO END -->

| Public release | Schema | Runtime | License |
| --- | --- | --- | --- |
| `v0.1.3` | `1.0` | Python standard library only | MIT |
...
FlowGuard is an architecture simulator and software workflow simulator used
before code is written. It does not make an AI agent write more code directly.
It gives the agent an environment for simulating the code path it is about to
create.

When an agent is about to build a workflow, refactor module boundaries, or
change retry, cache, deduplication, or idempotency behavior, FlowGuard asks it
to compress the planned program flow into a finite, executable, enumerable
model. FlowGuard then runs that model, keeps every trace, and checks invariants,
scenario expectations, loop/progress behavior, contracts, and implementation
conformance. The useful part is that workflow defects can appear before the real
code exists.
...
- It checks the state transitions of the whole workflow, not only whether one
  local function runs.
- It enumerates repeated inputs, branches, dead paths, loops, and state drift,
  then reports a reproducible counterexample trace.
- It fits naturally into Codex or another AI coding agent workflow: simulate
...
cache, deduplication, idempotency, or module-boundary code, use it to simulate
the architecture and check the flow.
```
...
counterexamples, and only then edit real code.
