...
def evidence_gates_from_process_like(evidence: Any) -> tuple[EvidenceGate, ...]:
    ...
    command = EvidenceGate(
        ...
        proof_evidence_ids=(proof_artifact or result_path,),
        metadata={"command": str(getattr(evidence, "command", ""))},
    )
    background_enabled = bool(getattr(evidence, "background", False))
    ...
    background = EvidenceGate(
        "process_background",
        "background",
        result_status=EVIDENCE_GATE_STATUS_PROGRESS_ONLY if progress_only else EVIDENCE_GATE_STATUS_PASSED,
        current=(not background_enabled) or (has_exit_artifact and has_result_artifact and not progress_only),
        metadata={
