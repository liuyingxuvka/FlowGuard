...
def _migrate_legacy_path_quality_revision_for_build(
    root: Path,
    head: Any,
    snapshot: ModelSystemSnapshot,
) -> ModelRevisionSet | None:
    """Migrate the retired v1 path-quality projection in-memory for one build.

    This is an upgrade-only bridge for the exact current authority artifact.
    It is deliberately not used by normal authority loading: once the new
    revision is activated, the retired nested schema is no longer readable.
    Only the old lightweight/no-trigger projection is mechanically migratable;
    an old deep payload must be regenerated by its native producer instead of
    being guessed here.
    """

    fingerprint = str(getattr(head, "accepted_revision_set_fingerprint", ""))
    if not fingerprint.startswith("sha256:"):
        return None
    path = root / ".flowguard" / "model-mesh" / "revisions" / (
        fingerprint.split(":", 1)[1] + ".json"
    )
    try:
        raw = json.loads(
    ...
    if not isinstance(raw, Mapping):
        return None
    subjects_raw = raw.get("path_quality_subjects")
    results_raw = raw.get("path_quality_results")
    if not isinstance(subjects_raw, list) or not isinstance(results_raw, list):
        return None
    if not subjects_raw and not results_raw:
        return None
    legacy_schema = "flowguard.model-path-quality.v1"
    if any(
        not isinstance(item, Mapping)
        or item.get("schema_version") != legacy_schema
        for item in (*subjects_raw, *results_raw)
    ):
        return None
    # Verify the immutable retired artifact before changing its projection.
    if raw.get("fingerprint") != fingerprint_value(
        {key: value for key, value in raw.items() if key != "fingerprint"}
    ):
        raise ModelAuthorityError(
            "retired path-quality revision fingerprint is stale or foreign"
        )
    subject_map: dict[str, PathQualitySubject] = {}
    subject_payloads: list[dict[str, Any]] = []
    for original in subjects_raw:
        item = dict(original)
        item.pop("fingerprint", None)
        item["schema_version"] = PATH_QUALITY_SCHEMA_VERSION
        subject = PathQualitySubject.from_dict(
            {
                **item,
                "fingerprint": fingerprint_value(item),
            }
        )
        subject_map[str(original.get("fingerprint", ""))] = subject
        subject_payloads.append(subject.to_dict())
    if len(subject_map) != len(subjects_raw):
        raise ModelAuthorityError(
            "retired path-quality revision contains duplicate subject identities"
        )

    result_payloads: list[dict[str, Any]] = []
    typed_results: list[PathQualityResult] = []
    for original in results_raw:
        item = dict(original)
        old_subject_fingerprint = str(item.get("subject_fingerprint", ""))
        subject = subject_map.get(old_subject_fingerprint)
        if subject is None:
            raise ModelAuthorityError(
                "retired path-quality result does not bind a retired subject"
            )
        if (
            item.get("mode") != "lightweight"
            or item.get("trigger_ids")
            or item.get("candidate_ids")
            or item.get("rewrite_rule_ids")
            or item.get("unresolved_ids")
        ):
            raise ModelAuthorityError(
                "retired deep or unresolved path-quality material requires a fresh producer run"
            )
        item.pop("fingerprint", None)
        item["subject_fingerprint"] = subject.fingerprint
        item["schema_version"] = PATH_QUALITY_SCHEMA_VERSION
        item.update(
            {
                "optimization_depth": "lightweight",
                "cost_dimensions": [],
                "cost_measurements": [],
        ...
        # Construct first so the v2 identity projection (not the wire shape
        # of ``cost_measurements``) supplies the new fingerprint.
        result = PathQualityResult(**item)
        typed_results.append(result)
        result_payloads.append(result.to_dict())

    migrated = dict(raw)
    migrated["path_quality_subjects"] = subject_payloads
    # ``ModelRevisionSet`` canonicalizes results by the *new* subject
    # fingerprint.  The retired v1 order cannot be reused after subject
    # fingerprints change, so sort the upgraded wire rows before deriving the
    # migrated revision identity.
    typed_results = tuple(sorted(typed_results, key=lambda item: item.subject_fingerprint))
    result_payloads = [item.to_dict() for item in typed_results]
    migrated["path_quality_results"] = result_payloads
    migrated["path_quality_result_set_fingerprint"] = (
        path_quality_result_set_fingerprint(
            migrated["required_path_quality_model_ids"],
            tuple(subject_map.values()),
            tuple(typed_results),
        )
    )
    migrated.pop("fingerprint", None)
    migrated["fingerprint"] = fingerprint_value(migrated)
    # The checked-in authority was produced by an earlier explicit intent
    # migration whose bootstrap receipt records the exact v2 source lineage.
    # Once the immutable source fingerprint and the v1 path-quality rows have
    # been verified above, parsing that receipt is permitted only inside this
    # versioned direct-to-current upgrader.  No normal authority reader sees
    # the scoped historical schema.
    current_view = raw.get("current_effective_intent_view")
    bootstrap_receipt = (
        current_view.get("bootstrap_receipt")
        if isinstance(current_view, Mapping)
        else None
    )
    source_revision_schema = (
        bootstrap_receipt.get("source_revision_schema", "")
        if isinstance(bootstrap_receipt, Mapping)
        else ""
    )
    if source_revision_schema == "flowguard.model_revision_set.v2":
        with allow_upgrade_bootstrap_source_schemas(source_revision_schema):
            revision = ModelRevisionSet.from_dict(migrated)
    else:
        revision = ModelRevisionSet.from_dict(migrated)
    if revision.candidate_snapshot_fingerprint != snapshot.fingerprint:
        raise ModelAuthorityError(
            "retired path-quality migration is not bound to the current authority snapshot"
        )
    return revision
...
def _load_current_accepted_revision_set_for_build(
    ...
    *,
    ...
    """Load current authority, permitting only the explicit v1 upgrade input."""
    ...
    try:
        revision = load_current_accepted_revision_set(
            ...
            head=head,
            snapshot=snapshot,
            # The builder is the one explicit upgrade-only caller.  It may
            # replay retired v1 path-quality rows in the immutable
            # predecessor chain while constructing the new current revision;
            # ordinary authority reads remain current-schema-only.
            allow_legacy_path_quality_upgrade=True,
        )
    except (ModelAuthorityError, ValueError) as original_error:
        migrated = _migrate_legacy_path_quality_revision_for_build(
            root,
            head,
            snapshot,
        )
