...
_MODEL_IDS = ("alpha", "beta")


def _write_consumer(root: Path) -> None:
    """Write a finite two-model consumer with real behavior and native cases."""
    ...
    for model_id in _MODEL_IDS:
        ...
        if model_id == "beta":
            input_paths.append("src/join.py")
        purpose = build_model_purpose_closure(
            ...
            claim_boundary=(
                "This fixture proves only the finite two-model consumer "
        ...
        json.dumps(
            {
                ...
                "claim_boundary": "Only this finite two-model consumer owner map.",
            },
            sort_keys=True,
        ),
        encoding="utf-8",
    ...
    semantic_mesh = {
        ...
        "whole_system_completion_claim": "Only the finite two-model consumer mesh is declared.",
...
def test_consumer_runners_execute_real_alpha_beta_cases_and_cross_model_connection(tmp_path: Path):
    target, staging = _consumer_roots(tmp_path)
    ...
    assert {
        "case:beta:join-negative",
        "case:beta:join-zero",
        "case:beta:join-positive",
    } <= set(results["beta"].executed_case_ids)
    assert all(item.native_case_verification and item.native_case_verification.ok for item in results.values())
    # The runner above imported the actual source functions and exercised the
    # connection cases; the parent receipt must therefore include the same
    # current two-owner composition.
    assert json.loads(
...
def test_affected_alpha_update_reuses_beta_validates_connection_and_activates_once(
    ...
    assert (
        "model_relation:relation:semantic-cross-boundary-support:alpha:parent:beta"
...
def test_required_connection_failure_is_terminal_and_preserves_current_pointer(
    ...
    assert results["beta"].status == "fail"
    assert results["beta"].producer_invocations == 1
    assert "model.nonzero_exit" in results["beta"].finding_codes
    assert manifest_path.read_bytes() == before_manifest
