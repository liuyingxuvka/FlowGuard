...
class MaintenanceScanTests(unittest.TestCase):
    def test_self_maintenance_requires_current_read_only_work_context(self):
        report = review_flowguard_self_maintenance(
            SelfMaintenancePlan(
                "spec-self-maintenance",
                required_work_context_ids=("openspec:change-one",),
            )
        )
        self.assertIn(
    ...
    def test_required_work_context_cannot_hide_behind_generic_green_evidence(self):
        ...
        exact = MaintenanceEvidence(
            ...
            work_context_hash="sha256:inputs",
