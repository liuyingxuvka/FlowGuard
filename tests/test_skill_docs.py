...
class SkillDocsTests(unittest.TestCase):
    def test_skill_mentions_state_inventory_and_targeted_conformance(self):
        ...
        self.assertIn("boundary_missing", text)
        self.assertIn("state_too_coarse", text)
        self.assertIn("input_branch_missing", text)
        self.assertIn("invariant_too_weak", text)
        self.assertIn("evidence_overclaimed", text)
        self.assertIn("same-class", text)
        self.assertIn("generalized bad case", text)
        self.assertIn("Generalized case", text)
        self.assertIn("registry, upgrade reviewer", text)
        self.assertIn("evidence-level field as a default response", text)
    ...
    def test_modeling_protocol_links_state_inventory(self):
        ...
        self.assertIn("boundary_missing", text)
        self.assertIn("same-class generalized bad case", text)
        self.assertIn("Record `Miss type` and `Generalized case`", text)
    ...
    def test_agents_snippet_carries_risk_purpose_header_rule(self):
        ...
        self.assertIn("boundary_missing", text)
        self.assertIn("same-class generalized bad case", text)
        self.assertIn("evidence-level field as", text)
        self.assertIn("Miss type", text)
        self.assertIn("Generalized case", text)
        self.assertIn("current-project model names", text)
        self.assertIn("Preserve user and peer-agent changes", text)
