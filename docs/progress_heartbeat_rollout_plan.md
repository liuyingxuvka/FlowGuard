# Internal Finite-Runner Progress Heartbeat Rollout Plan

This document is the working checklist for adding minimal progress visibility to
long-running FlowGuard finite model runs.

## Optimization Checklist

| Order | Optimization item | Implementation boundary | Done when |
| --- | --- | --- | --- |
| 1 | Add default bounded progress to the internal finite-runner path. | `flowguard/explorer.py` only. Count top-level work as `initial_state x input_sequence`. | Formal model checks emit start and progress lines without requiring caller changes. |
...
| 5 | Let `run_model_first_checks()` inherit core behavior. | Do not add duplicate progress logic to `flowguard/runner.py`. | Runner calls show finite-runner progress through the same core path. |
| 6 | Update templates and docs by reference, not by copying logic. | Keep generated templates on `run_model_first_checks(...)` so they inherit core finite-runner progress through the formal runner. | New templates rely on the core feature instead of writing their own progress loop. |
...
| --- | --- | --- | --- |
| R1 | The plan adds progress outside the core finite runner, so old models do not benefit. | Require `core_explorer_changed` and reject `template_only_progress`. | Formal model checks emit progress through the shared runner. |
...
| R8 | Runner gains a second inconsistent progress implementation. | Require `runner_inherits_core` and reject duplicate runner progress. | `run_model_first_checks()` still delegates to the finite runner without custom progress code. |
...
- No parallel finite-runner execution in this release.
