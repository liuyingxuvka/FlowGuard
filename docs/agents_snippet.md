...
| Canonical finite bad-case generation from declared fields, state/input boundaries, Cartesian interaction groups, same-class families, payload contracts, transition cells, parent/child closure, or no-delta loops | `contract_exhaustion_mesh` | `flowguard-contract-exhaustion-mesh` |
