...
Never select by keyword score or declaration order. A narrow entry reduces initial reading; it does not reduce the final model depth.
...
Read the selected skill's `Local Material Routing`. Its first protocol owns route detail. Load deeper modeling, evidence, templates, commands, or examples only when the selected route names their trigger; do not eagerly read unrelated route protocols.
