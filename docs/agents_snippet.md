...
- In an existing modeled project, audit the sole content-addressed `observed_implementation` snapshot before treating a model as current. `normative_target` and `counterfactual_experiment` stay isolated; filenames, prompt labels, discovery hits, and green candidate checks do not grant authority. Missing/invalid authority or unresolved required coverage blocks broad current-model confidence.
- Replace the observed model system only through one accepted `ModelRevisionSet` bound to the exact base, candidate, affected closure, changed models/relations/fields/effects/contracts/tests, prediction/replay evidence, and current owner receipts. Write immutable records first and move the sole project pointer last under shared compare-and-swap locking. Rollback restores or compensates real implementation effects and revalidates the old snapshot before moving authority; irreversible effects require forward repair.
...
| Current software-model authority, target/experiment isolation, whole revision activation, or operational rollback | `model_mesh_maintenance` | `flowguard-model-mesh` with `model-system-audit`, `model-system-bootstrap`, `model-revision-activate`, or `model-revision-rollback` |
