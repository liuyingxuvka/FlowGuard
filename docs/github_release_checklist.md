...
- `flowguard/`
- tracked `.flowguard/` executable model sources, `project.toml`, and the
  model-regression manifest;
...
Do not publish local derived maintenance artifacts:

- `.flowguard/evidence/`, generated reports/results, progress files, and local
  receipt stores;
- `kb/`
- `PLAN.md`
- local adoption logs;
- local adoption review reports;
- generated bytecode or cache files (intentional checked-in test fixtures are
  allowed only when their fixture ownership is explicit);
- private pilot notes;
...
- `.flowguard/evidence/`, generated local `.flowguard` reports/results, or
  `kb/` is present in tracked or staged release files;
