...
FlowGuard v0.65.0 is source-only. Bind local release readiness to the exact
...
```powershell
python scripts/verify_flowguard_release.py --root . --phase tag --tag v0.65.0 --parent-receipt <parent-receipt-id> --receipt-root .flowguard/evidence/validation-owners --json
```
...
```powershell
python scripts/verify_flowguard_release.py --root . --phase published --tag v0.65.0 --parent-receipt <parent-receipt-id> --receipt-root .flowguard/evidence/validation-owners --repository liuyingxuvka/FlowGuard --json
```
...
FlowGuard v0.65.0 只发布源码。本地发布结论必须绑定当前 8 个验证负责人组成的
