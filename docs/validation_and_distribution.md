...
FlowGuard is an AI-agent skill suite powered by an executable check engine. This guide explains how to validate the current repository without confusing progress with proof, and how to install or compare the complete 17-member skill tree without overwriting user-owned files.
...
FlowGuard 是一套由可执行检查引擎驱动的 AI-agent 技能套件。本说明告诉你怎样验证当前仓库而不把“还在运行”误当成“已经证明”，也说明怎样安全安装或比较完整的 17 项技能树，而不覆盖用户自己的文件。
