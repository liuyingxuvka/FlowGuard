...
FlowGuard v0.60.0 is source-only. Bind local release readiness to the current
...
```powershell
python scripts/verify_flowguard_release.py --root . --phase published --tag v0.60.0 --repository liuyingxuvka/FlowGuard --json
```
...
FlowGuard v0.60.0 只发布源码。把本地发布结论绑定到当前统一门禁的 8 个子结果，
