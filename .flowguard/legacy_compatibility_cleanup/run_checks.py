...
def main() -> int:
    workflow_checks = (
        run_workflow("correct_legacy_compatibility_cleanup", model.build_correct_workflow(), expect_ok=True),
        run_workflow("broken_deletes_safety_classifier", model.build_broken_safety_workflow(), expect_ok=False),
        run_workflow(
            "broken_skips_installed_skill_parity",
            model.build_broken_installed_skill_workflow(),
            expect_ok=False,
        ),
    )
    reports = (
