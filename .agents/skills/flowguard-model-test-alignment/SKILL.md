...
## Purpose
Compare model obligations, external `CodeContract`, bindings, and current test coverage.
...
2. Convert pre-code rows into obligations, contracts, targets, oracle/test designs, bad cases, or scoped dispositions; `planned`/`not_run` never pass.
...
- Omitted surfaces, unresolved dispositions, orphan helpers, hidden writers, duplicate bindings, stale fingerprints, or missing semantics/oracles block blueprint alignment; it never reconstructs.
- Bind evidence and recommendations to exact current model, maturation, and admission identities; drift makes them stale.

## Output Requirements
- Return `evidence`, `failures`, `blockers`, `skipped_checks`, `residual_risk`, `claim_boundary`, `typed_next_actions`, pre-code/executed status, binding gaps, and a diagram whose edges mean covers, partially covers, or does not cover. Blueprint output adds fingerprints, exact surface ids, semantic/oracle gaps, and a static-only claim boundary.
