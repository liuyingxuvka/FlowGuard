...
## Entrypoint Scope
Owner of hierarchy; not test/code splits.

## Local Material Routing
After selecting the subject read `references/model_mesh_protocol.md`; load `references/model_mesh_partition_protocol.md`, `references/model_mesh_reattachment_protocol.md`, or `references/model_mesh_closure_protocol.md` only when triggered.
...
## Output Requirements
- Return Child Reattachment Gate, siblings/gaps/receipts, and mesh diagram; edges mean delegates, reattaches, consumes output, realizes, or blocks. Blueprint adds depth/gap; compact reads retain denominators, fingerprints, stale/not-run markers, and omitted ids.
