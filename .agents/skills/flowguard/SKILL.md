...
5. Preserve checked/unchecked/blocked/scoped evidence and close only at the requested current scope. For non-trivial model work, always attempt the model's highest useful task-local depth: derive independent coverage, predict a discriminating outcome, act on the open gap, and repeat native checks until `model_closed_for_task` or an explicit external/scope/stall/limit terminal reason. Never ask the model to self-rate understanding; self-report is not evidence.
