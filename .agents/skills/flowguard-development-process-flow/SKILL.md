...
- Post-code, post-miss, model-test, mesh, code-boundary, or freshness signals
  that imply the model is too coarse must pass through
  `review_model_maturation_loop(...)` before broad done/release confidence.
...
- Before broad done/release/archive/publish/framework-sync confidence, use
  proof-artifact-bound evidence. A validation row that only declares
  `passed/current` is not enough without a current `ProofArtifactRef` carrying
  result path, fingerprint, covered validation obligation, and no route gaps.
...
2. Record which validations cover which artifact versions.
   For broad claims, attach the proof artifact generated by each command,
   replay, or evidence collector.
...
17. Before final done/release/archive/publish, consume
    ...
    probes.
18. For non-trivial lifecycle reviews, default to a user-facing Mermaid
