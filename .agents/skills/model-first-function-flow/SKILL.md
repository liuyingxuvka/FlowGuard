...
This is the default entrypoint for the FlowGuard AI-agent skill suite. Complete setup means this skill and all sibling FlowGuard skills under `.agents/skills/` are available to the host agent.

This compact router owns hard gates and route selection. Keep details in `references/*.md` or the matching satellite skill; use a direct satellite when obvious, and this kernel for ordinary modeling, unclear routing, cross-route coordination, or core model work before narrowing.

Skill availability and executable evidence are separate: if skills are visible but checks cannot run, preserve the route decision and report executable evidence as blocked, partial, or scoped.
...
When check-engine helpers are needed, read `AGENT_DEFAULT_API`, then the selected
...
1. Decide applicability/lens, verify the check engine/adoption record for real project work, and model the risky finite-state boundary.
