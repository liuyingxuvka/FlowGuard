...
Required behavior-plane, typed-relation, lookup, preflight, similarity, former-shape rejection, and Model Miss obligations SHALL each bind one owner public code contract and that owner's existing native tests covering the same contract.
...
### Requirement: Replacement evidence covers source and disposition
Replacement obligations SHALL bind canonical loading, former Python/JSON/field rejection, current semantic coverage, and old-authority retirement to the ledger owner's public contract and native tests.
...
- **THEN** alignment SHALL keep the replacement closure target open
