...
- [x] Work in the isolated FlowGuard worktree D:\FlowGuardJointAuditRepo_20260920 on refactor/compact-routes-20260920; baseline v0.69.5 is 81e34ae7a626b949935f73c76a46931bba554aa1.
- [x] Record the cross-repository SkillGuard change at D:\SkillGuardJointAuditRepo_20260920\openspec\changes\close-compact-runtime-acceptance; historical changes were not edited.
- [x] Use the prepared CPython 3.12 environment and current repository identity for all final checks; unexpected writes were treated as evidence drift and the final model was regenerated after tracked changes.
...
- [x] Retain typed native cases and boundary cases; native producers do not infer qualification from stdout or global state.
- [x] Bind retained native leaves to one current owner and reject missing, duplicate, malformed, or failed case results.
- [x] Generate and execute the prepared native plan with the current case mapping; targeted native/lifecycle suites pass and no external target is substituted by Python.
- [x] Record the final native/model receipts under work/verification/joint-20260920; raw work output remains outside the published source package.

## C05 — Complete writer-bound selected map reads
...
- [x] Rebuild the current model from source inputs, relations, recursive leaf identities, and retired-wrapper dispositions; the final parent regression executes 51 owners with passed=51, failed=0, skipped=0, reused=0.
- [x] Preserve functional-input and child-execution-key identity for independent leaves; unrelated authority-head changes do not invalidate an unchanged leaf.
- [x] Keep historical wrappers and scheduler material out of the public consumer surface; retire only explicitly mapped physical paths after replacement checks pass.
- [x] Freeze the final reverse discovery/map identity after source changes: discovery surface_count=17158, semantic map surface_count=17158, and 25 model obligations are current.
...
## C08 — Reconcile every migrated test and CI denominator
