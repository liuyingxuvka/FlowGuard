...
progress-only, internal-path-only, not-run, and explicitly out-of-scope
evidence before full confidence is claimed.
...
#### Scenario: Explicitly out-of-scope row is visible
- **WHEN** a risk row is marked out of scope with a reason
- **THEN** the ledger review reports scoped confidence rather than hiding the
  unproven risk
