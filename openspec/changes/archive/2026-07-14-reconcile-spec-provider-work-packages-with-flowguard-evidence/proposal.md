...
- Replace all seventeen managed FlowGuard skills directly with one current contract trio and reject every former runtime surface. No migration reader, fallback route, or second runtime authority remains.
...
- `flowguard-skill-suite-distribution`: Preserve one current contract trio, generated-contract parity, installed-layout parity, and residual-absence checks for all managed skills.
...
- Narrow updates to existing process, test-mesh, plan-detail, preflight, post-change routing, release verification, skill contracts, and current OpenSpec verification contracts.
