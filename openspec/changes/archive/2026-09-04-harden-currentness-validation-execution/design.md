...
| terminal merged-owner handoff | 11 | Preserve the exact historical owner and delta identity while consuming the already-current inventory, contract-governance, distribution, retired-entry, and project-adoption targets. The stale owner delta is archived with `--skip-specs` and is never applied again. |
...
The 11 terminal handoff rows are similarly constrained. They converge on one packaged
...
- `external_owner`, with the exact historical owner change and terminal merged
  handoff into an already-current target requirement.

`undecided`, duplicate keys, missing archive inputs, or a non-terminal
`external_owner` block archive and broad confidence. A terminal merged handoff
retains the old owner identity only as provenance and requires `--skip-specs`
retirement of its stale delta. The final ledger contains
...
The 11 historical merge rows have a terminal documented handoff from
`remove-project-upgrade-author-suite-dependency`. The ledger names its exact two
delta files, the current target specs, the eleven-row count, and
`archive_mode=skip_specs`. The active stub must not apply its stale delta: all
target requirement names are already current, its removal source is absent, and
one of its additions already exists. The semantic-sync parent keeps the rows
terminal only while those current targets and the handoff metadata remain
exact.

`repair-consumer-authority-v06814` is separately superseded. Its one
version-neutral packaged-authority requirement is transferred into the current
`flowguard-skill-suite-distribution` delta with all three scenarios; none of its
old `0.68.14` version, installation, tag, publication, or remote-readback steps
becomes current work. That historical change is also eligible only for
`--skip-specs` retirement after the receiving delta passes strict validation.
...
2. Add the semantic-sync ledger/checker and reconcile current specification
   authority, including the 12 restorations and the exact terminal merged-owner
   handoff of the 11 rows.
...
10. Finish and archive this source-controlled change.  The release checklist
   then freezes the archived source, model authority, projections, and toolchain
   and runs exactly one post-archive final full parent.  Commit, tag, publish,
   post-publication comparison, and predictive-KB postflight are immutable
   output gates; they must never be recorded by editing this change's task
   checkboxes after that parent has run.
...
3. Verify the recorded terminal handoff for the 11 suite/project rows: exact
   old delta files, all current target requirements, and `--skip-specs` archive
   mode. Any drift returns the handoff to blocked.
...
4. Consume the 11 suite/project outcomes from their current target requirements
   and retain the old owner delta as non-applicable transfer evidence.
...
| A target requirement, old delta file, or handoff field for one of the 11 rows changes | Invalidate the consuming plan and keep those rows blocked. Reconcile the exact current target and re-freeze the explicit handoff; never apply the stale delta as fallback. |
...
- The terminal handoff for `remove-project-upgrade-author-suite-dependency`
  preserves its two old delta files as read-only provenance; no agent may apply
  them or edit the eleven current targets through that retired change.
...
After every functional input is frozen and this change has been archived, the
release checklist plans the complete owner DAG once. Focused and affected
checks may have produced reusable child receipts, but only one full parent
producer may run for the frozen identity. The final test
asserts:
...
### Recursive model/test closure

The model and test layers use one canonical post-order hierarchy. A non-leaf
node is never complete from an ordinary `passed` flag; it consumes an exact
current canonical child subtree receipt. Leaf input/state axes and product
signatures are generated from the kernel, and parent reports recompute local
and descendant totals. `ModelRevisionSet` binds the hierarchy fingerprint and
root subtree receipt together with the flat model snapshot so a subtree cannot
be activated independently of the observed authority.

The local Cartesian product remains bounded. A model-local product must contain
only axes for that model; cross-model combinations use a typed interface
product and an explicit refinement contract. Missing axes, a split singleton
group, a foreign axis, a ghost child receipt, a duplicate leaf owner, or a
truncated product are blockers for broad claims.

### Completion epoch

The final validation owner creates one immutable completion epoch only after
all governed source writes, OpenSpec edits, model changes, test changes, skill
projection changes, and external-root synchronization are settled and the
source-controlled change has been archived. A terminal ledger is output-only
and is not part of the governed source manifest. One epoch may run at most one
full producer DAG. Source drift aborts that epoch and requires a new epoch after
a batched repair; output-only receipt or progress writes do not create drift.
Live source reconciliation occurs before parent publication, followed only by
artifact-integrity verification. The archive boundary is what prevents a
post-final task checkbox edit from invalidating the parent or creating a
self-referential freshness loop.

### Reverse input boundary

The authored reverse semantic map is a functional input, not a validation
output. Its exact canonical fingerprint is part of the reverse owner current
identity. Discovery snapshots, reports, receipts, and progress remain output
only. An authored-pending-audit map cannot authorize a broad reverse closure;
the semantic owner must provide the independent acceptance receipt.
