...
the release version, documentation, OpenSpec change archive, current model
authority, and consumer installation/parity are final. Exactly one
post-archive final full parent gate SHALL consume that frozen identity set; it
MAY execute stale or missing owners and reuse independently verified
exact-current receipts. The command MUST publish broad success only as an
independently verified receipt whose subject is `validation-parent:full`.
Commit, tag, push, and publication SHALL occur only after that parent passes,
and no OpenSpec task checkbox or governed source file may be edited after the
parent is published in the same completion epoch.
...
- **THEN** the prior final parent is invalid, publication is blocked, and a new
  manifest set and affected owner plan are required; changing a completed task
  checkbox cannot repair or refresh the old parent
