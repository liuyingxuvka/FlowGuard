...
This implementation is also the current authority for the recursive model/test
closure and completion-epoch repair discovered during the audit. It must make
the existing model, code, test, and reverse-surface layers consume one another
through exact current receipts; it must not add a parallel compatibility path
or rely on a final task-file edit after validation.
