...
- Add one canonical machine-readable project ledger and boundary-only migration from the current Python-authored template shape; do not keep two active ledger authorities.
...
- Update current project rules, FlowGuard skills, prompt metadata, templates, self-models, field lifecycle, contract-exhaustion cases, model-test bindings, tests, installation parity, and project-upgrade evidence.
- **BREAKING**: normal runtime commitment rows use typed `relations` instead of `dependency_commitment_ids`; legacy artifact input is accepted only at the explicit upgrade boundary and is not a second successful runtime path.
...
- `behavior-commitment-ledger`: Add behavior-plane, actor-kind, typed-relation, lookup-binding, canonical-ledger, and migration requirements.
...
- `field-lifecycle-mesh`: Account new persisted, prompt, lookup, relation, migration, and preflight fields plus the closing disposition of the old dependency field.
- `contract-exhaustion-mesh`: Generate finite plane/actor/relation/lookup/migration known-bad cases and stable downstream case ids.
- `model-test-alignment`: Bind plane-aware model obligations to the owner public code contracts and current tests.
- `test-evidence-mesh`: Keep focused, background, install-parity, model-regression, and full-suite evidence visible and freshness-bound.
- `development-process-flow`: Preserve ordering and freshness across OpenSpec, model, schema, migration, code, prompt-contract compilation, installation, and validation without absorbing product or AI-operation ownership.
...
- Core modules: `behavior_commitment.py`, a new `behavior_commitment_lookup.py`, `existing_model_preflight.py`, `model_similarity.py`, `recurring_model_miss.py`, `artifact_upgrade.py`, `templates.py`, `__init__.py`, and `__main__.py`.
...
- Evidence: focused unit tests, migration dry-run/apply tests, ContractExhaustion cases, Model-Test Alignment bindings, affected/full model regressions, full test suite, project audits, OpenSpec verification, and local release-readiness evidence. No external publication or Git operation is implied by this change.
