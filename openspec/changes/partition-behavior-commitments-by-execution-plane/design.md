...
- Replace the Python-authored project ledger with one canonical data artifact and one boundary upgrader.
...
- Produce current focused, model-regression, full-suite, migration, and OpenSpec verification evidence.
...
- No mass rename of existing commitment ids in the first migration.
...
An AI feature implemented inside the product is `product_runtime`; `agent_operation` means the development/maintenance agent acting to complete the current task. A migration-only `unclassified` value may appear in an upgrade report but cannot be accepted by the runtime ledger or broad confidence review.
...
Reverse context is derived by the lookup helper; duplicate inverse rows are not required. Cross-plane rows require a non-empty rationale. Legacy `dependency_commitment_ids` are accepted only by the upgrader. Same-plane deterministic dependencies become `depends_on`; ambiguous or cross-plane legacy rows require review.
...
The existing project `model.py` becomes a thin loader/adaptor, and `run_checks.py` writes only result evidence. The runtime does not execute arbitrary Python to discover ledger data. The upgrader auto-converts only the known official Python template shape or explicit legacy JSON mappings; unknown custom Python receives a manual-conversion item and is not executed.
...
- `BehaviorLookupReport`: status, selected/ambiguous plane, primary hits, related hits, ledger fingerprint, and fallback reason.
...
Only primary-plane hits can guide same-plane operation. Related hits are labelled as invoked targets, governing process, validation target, or evidence source. A missing/stale ledger keeps the path scan available as an explicit fallback and cannot be described as full behavior lookup.
...
- `artifact_upgrade.py`: boundary-only legacy conversion.
...
FieldLifecycleMesh accounts every added field and the removed legacy dependency field. ContractExhaustionMesh declares finite axes for plane, actor kind, relation type/direction, lookup state, and migration state. Model-Test Alignment binds each required obligation to the public owner contract and current tests. TestMesh keeps focused, affected-model, skill/install parity, and full/background validation separate; progress is never pass evidence.
...
- **Risk: custom Python ledgers cannot be converted automatically** → Never execute them; produce a manual migration report with old-source retirement still open.
...
- **Risk: background regressions are mistaken for completion** → TestMesh/DevelopmentProcessFlow record running jobs as liveness only; closure consumes final exit/result artifacts.

## Migration Plan

1. Run project and installed-tool preflight; freeze current ledger and skill fingerprints.
2. Add schema/review types while preserving old artifact reading only in the upgrader.
3. Add canonical serializer/loader and generate a dry-run conversion report.
4. Convert the official project ledger to `ledger.json`; change `model.py` into a loader; verify semantic parity.
...
8. Install to a shadow target, verify source/compiled/installed parity, and only then update the formal installed suite.
9. Run full model regressions and tests in background, consume final receipts, rerun project/OpenSpec verification after the last peer/shared write, and perform a requirement-by-requirement completion audit.

Rollback restores the matched previous package/ledger/skill-install snapshot as one unit. It must not activate the old Python ledger beside the new JSON ledger. A failed migration leaves the old authority untouched and the new artifact inactive.
